...
- README delivery: commit `f0d86fc` (`docs(readme): document gateway usage and boundaries`) expands the root README and corrects the active-health support matrix wording. The push was attempted twice and is currently blocked by a connection reset/timeout to `github.com:443`; the commit is included in the local feature branch and will be included in the requested merges.
