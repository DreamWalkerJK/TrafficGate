...
public sealed class GatewayMetrics
{
    ...
    long proxyRequestCount, proxyErrorCount, throttledCount, forwardingFailureCount;
    ...
    public void ProxyRequest(string route, string outcome, double gatewayMs, double upstreamMs)
    {
        ...
        Interlocked.Increment(ref proxyRequestCount);
        proxyRequests.Add(1, tags); gatewayDuration.Record(gatewayMs, tags); upstreamDuration.Record(upstreamMs, tags);
        if (outcome is "rate_limited" or "concurrency_limit" or "partition_limit") { Interlocked.Increment(ref throttledCount); throttled.Add(1, tags); }
        if (outcome is not "success" and not "upstream_error") { Interlocked.Increment(ref proxyErrorCount); proxyErrors.Add(1, tags); if (outcome.StartsWith("upstream_", StringComparison.Ordinal) || outcome == "no_healthy_destinations") { Interlocked.Increment(ref forwardingFailureCount); forwardingFailures.Add(1, tags); } }
    }
    public string Snapshot() => $"# TYPE trafficgate_proxy_requests_total counter\ntrafficgate_proxy_requests_total {Volatile.Read(ref proxyRequestCount)}\n# TYPE trafficgate_proxy_errors_total counter\ntrafficgate_proxy_errors_total {Volatile.Read(ref proxyErrorCount)}\n# TYPE trafficgate_throttled_requests_total counter\ntrafficgate_throttled_requests_total {Volatile.Read(ref throttledCount)}\n# TYPE trafficgate_forwarding_failures_total counter\ntrafficgate_forwarding_failures_total {Volatile.Read(ref forwardingFailureCount)}\n";
}
